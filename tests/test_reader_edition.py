...
def test_reader_build_names_evidence_coverage_and_rejected_exposure(tmp_path):
    ...
    assert written["evidence_coverage"] == {
        "neutral": {"normalized": 1015, "texts": 1101},
        "languages": {
            "en": {"normalized": 112, "effective": 1096, "texts": 1101},
            "pl": {"normalized": 72, "effective": 1064, "texts": 1101},
        },
    }
...
def test_the_index_compresses_to_something_a_phone_can_hold(tmp_path):
    ...
    assert packed < 650_000, "the index is what search and lemma pages both read"
...
def test_language_indexes_are_small_and_independently_packaged(tmp_path):
    ...
    for language in ("pl", "en"):
        ...
        assert packed < 350_000, f"{language} search index is too large ({packed})"
        manifest = json.loads(
...
def test_metrics_are_the_single_derived_denominator(tmp_path):
    ...
    assert metrics == {
        ...
        "texts": {"total": 1101, "proprium": 996, "words": 61960, "verse_segments": 2607},
        "languages": {"en": {"texts": 1101}, "pl": {"texts": 1101}},
        "formularies": {
            "total": 109,
            "observances": 105,
            "component_uses": 1213,
            "unique_component_texts": 1019,
        },
    }
