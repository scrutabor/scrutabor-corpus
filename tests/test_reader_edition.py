...
def test_reader_build_names_evidence_coverage_and_rejected_exposure(tmp_path):
    ...
    assert written["evidence_coverage"] == {
        "neutral": {"normalized": 740, "texts": 840},
        "languages": {
            "en": {"normalized": 108, "effective": 826, "texts": 840},
            "pl": {"normalized": 72, "effective": 791, "texts": 840},
        },
    }
...
def test_language_indexes_are_small_and_independently_packaged(tmp_path):
    ...
    for language in ("pl", "en"):
        ...
        assert packed < 250_000, f"{language} search index is too large ({packed})"
        manifest = json.loads(
...
def test_metrics_are_the_single_derived_denominator(tmp_path):
    ...
    assert metrics == {
        ...
        "texts": {"total": 840, "proprium": 743, "words": 46163, "verse_segments": 1805},
        "languages": {"en": {"texts": 840}, "pl": {"texts": 840}},
        "formularies": {
            "total": 81,
            "observances": 77,
            "component_uses": 902,
            "unique_component_texts": 758,
        },
    }
