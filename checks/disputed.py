...
def collect() -> dict[str, list[tuple[str, str, str | None, str | None]]]:
    """form -> [(text id, word id, review, confidence)], for every token the
    corpus does not stand fully behind.

    A token qualifies on EITHER count, so the other may be unset: a word
...
def main(argv: list[str]) -> int:
    ...
    for form, where in sorted(found.items(), key=lambda kv: (-len(kv[1]), kv[0])):
        # an unset half prints as a dash, not as the word None
        flags = {f"{r or '—'}/{c or '—'}" for _, _, r, c in where}
